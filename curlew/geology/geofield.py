...
class Geode( object ):
    ...
    Attributes:
        ...
        property (torch.tensor or np.ndarray): (N,d) array of property values derived at each `x`.
        propertyNames (list): List of `d` property names corresponding to each dimension of `self.property`.
    ...
    property : torch.tensor = None # forward (property) predictions
    ...
    def combine(self, younger, weight):
        ...
        if self.property is not None:
            ...
            args['property'] = self.property.copy() # default to the older property values
            if younger.property is not None: # overprint with younger values if defined
                args['property'] = younger.property*weight + args['property']*iweight
        elif younger.property is not None:
            args['propertyNames'] = younger.propertyNames
            args['property'] = younger.property*weight
...
class GeoField( object ):
    ...
    def __init__( self, 
        ...
        property : curlew.geology.forward.ForwardModelBase, None
            A custom forward function that translates the implicit scalar field values into estimates of some measured
            property (e.g., density, mineralogy, etc.). See `curlew.geology.forward` for details, including implementations
        ...
        self.property = property # forward (property) prediction, if defined
        ...
        self._lastScalar = None # temporary storage for last evaluated scalar field (used to retrieve results from e.g., faults)
        self._lastDisp = None # temporary storage for last evaluated displacement field (used to retrieve results from e.g., faults)
    ...
    def predict(self, x: np.ndarray, combine=False, to_numpy=True, transform=True, values=None, 
        ...
        if combine and (self.parent is not None): # COMBINE RESULTS FROM MULTIPLE FIELDS?
            if self.parent2 is not None: # DOMAIN BOUNDARIES (essentially use this field as a mask to combine older fields)
                ...
                parent = self.parent.predict( x, combine=True, to_numpy=False, transform=True)
                ...
                if self.overprint is not None: 
                    ...
                    # evaluate this field
                    child = self.predict( x, combine=False, to_numpy=False, transform=True )
            ...
            propertyNames=[]
            if self.property is not None:
                # TODO -- apply property model if defined
                pass
            ...
            out = Geode(x=x, grid=grid, time=time, 
                        ...
                        scalar=scalar, structureID=structureID, structureLookup=structureLookup,
        ...
        if self._lastDisp is not None:
            out.offsets[self.name] = self._lastDisp
            self._lastDisp = None
        
        # also store values used to compute displacement field too
        if self._lastScalar is not None: 
            out.fields[self.name] = self._lastScalar
            self._lastScalar = None
